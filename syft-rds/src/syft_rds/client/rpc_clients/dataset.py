...
class DatasetRPCClient(CRUDRPCClient[Dataset, DatasetCreate, DatasetUpdate]):
    """
    Dataset works only locally (not sending any RPC requests)
    """
    ...
    def __init__(self, config: "RDSClientConfig", connection: RPCConnection):
        ...
        self._syftbox_client = connection.sender_client
        self._path_manager = DatasetPathManager(self._syftbox_client, self.config.host)
