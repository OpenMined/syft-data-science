from typing import Annotated

from fastapi import APIRouter, Depends
from syft_core import Client as SyftBoxClient
from syft_core import SyftBoxURL

from syft_rds.models import (
    GetAllRequest,
    GetOneRequest,
    ItemList,
    UserCode,
    UserCodeCreate,
    UserCodeUpdate,
)
from syft_rds.server.user_file_service import UserFileService
from syft_rds.server_fsb.dependencies import (
    get_current_user,
    get_syftbox_client,
    get_user_code_store,
    get_user_file_service,
)
from syft_rds.store import YAMLStore
from syft_rds.utils.zip_utils import extract_zip

user_code_router = APIRouter()


@user_code_router.post("/create")
def create_user_code(
    create_request: UserCodeCreate,
    user_code_store: Annotated[YAMLStore[UserCode], Depends(get_user_code_store)],
    user_file_service: Annotated[UserFileService, Depends(get_user_file_service)],
    user: Annotated[str, Depends(get_current_user)],
    syftbox_client: Annotated[SyftBoxClient, Depends(get_syftbox_client)],
) -> UserCode:
    create_request.name = create_request.name or "My Code"
    user_code = create_request.to_item(extra={"created_by": user})
    user_code_dir = user_file_service.dir_for_item(user=user, item=user_code)

    if create_request.files_zipped is not None:
        extract_zip(create_request.files_zipped, user_code_dir)

    user_code.dir_url = SyftBoxURL.from_path(user_code_dir, syftbox_client.workspace)
    return user_code_store.create(user_code)


@user_code_router.post("/get_one")
def get_user_code(
    req: GetOneRequest,
    user_code_store: Annotated[YAMLStore[UserCode], Depends(get_user_code_store)],
    user: Annotated[str, Depends(get_current_user)],
) -> UserCode:
    filters = req.filters
    if req.uid is not None:
        filters["uid"] = req.uid
    item = user_code_store.get_one(**filters)
    if item is None:
        raise ValueError(f"No UserCode found with filters {filters}")
    return item


@user_code_router.post("/get_all")
def get_all_user_codes(
    req: GetAllRequest,
    user_code_store: Annotated[YAMLStore[UserCode], Depends(get_user_code_store)],
    user: Annotated[str, Depends(get_current_user)],
) -> ItemList[UserCode]:
    items = user_code_store.get_all(
        limit=req.limit,
        offset=req.offset,
        order_by=req.order_by,
        sort_order=req.sort_order,
        filters=req.filters,
    )
    return ItemList[UserCode](items=items)


@user_code_router.post("/update")
def update_user_code(
    update_request: UserCodeUpdate,
    user_code_store: Annotated[YAMLStore[UserCode], Depends(get_user_code_store)],
    user: Annotated[str, Depends(get_current_user)],
) -> UserCode:
    existing_item = user_code_store.get_by_uid(update_request.uid)
    if existing_item is None:
        raise ValueError(f"UserCode with uid {update_request.uid} not found")
    updated_item = existing_item.apply_update(update_request)
    return user_code_store.update(updated_item.uid, updated_item)
